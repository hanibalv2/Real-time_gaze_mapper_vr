...
fixed4 frag(g2f i) : SV_Target
{
    ...
    //fixed4 finalColor = lerp(_BaseColor, _WireColor, t);
                const fixed4 colors[11] = {
						...
						fixed4(1.0, 0.0, 1.0, 1.0),  // not so White
						fixed4(0.9, 0.0, 0.9, 1.0),  // not so White
						fixed4(0.8, 0.0, 0.8, 1.0),  // not so White
						fixed4(0.7, 0.0, 0.7, 1.0),  // not so White
						fixed4(1.0, 1.0, 1.0, 1.0),  // not so White
						fixed4(0.0, 1.0, 0.0, 1.0),  // not so White
						fixed4(0.4, 0.0, 0.4, 1.0),  // not so White
						fixed4(0.3, 0.0, 0.3, 1.0),  // not so White
						fixed4(0.2, 0.0, 0.2, 1.0),  // not so White
						fixed4(0.0, 0.0,1.0,0.1)  // not so White
/*
...
//int index = clamp(floor(cameraToVertexDistance), 0, 10);
  //              fixed4 wireColor = colors[index];
				fixed4 wireColor=lerp (float4(1,1,1,1),float4(0,0,0,1),cameraToVertexDistance/8);
